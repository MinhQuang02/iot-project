...
const ChatBot = () => {
    ...
    const handleSend = () => {
        ...
        setTimeout(() => {
            const botReply = {
                id: Date.now() + 1,
                text: "This is an automated reply from GreenSphere AI.",
                sender: 'bot',
                time: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })
            };
                ...
                {messages.map((msg) => (
                    <div key={msg.id} className={`flex gap-3 items-start ${msg.sender === 'user' ? 'flex-row-reverse' : ''}`}>
                        ...
                        <div className={`w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 ${
                            msg.sender === 'bot' ? 'bg-blue-100' : 'bg-brand-light'
                        }`}>
                        ...
                        <div className={`max-w-[75%]`}>
                            ...
                            <div className={`p-3 text-sm text-slate-700 shadow-sm ${
                                msg.sender === 'bot' 
    ...
    if (isMobile) {
        return (
            <>
                ...
                <div className={`fixed bottom-6 right-6 z-50 w-[90%] max-w-[350px] h-[450px] bg-white rounded-2xl shadow-2xl flex flex-col overflow-hidden transition-all duration-300 origin-bottom-right ${
